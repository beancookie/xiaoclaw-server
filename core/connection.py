...
class ConnectionHandler:
    ...
    def _initialize_memory(self):
        ...
        # 如果 LLM 未启用，跳过记忆初始化
        if self.llm is None:
            self.logger.bind(tag=TAG).info("LLM 未启用，跳过记忆模块初始化")
            return
        """初始化记忆模块"""
    ...
    def chat(self, query, depth=0):
        # 如果配置关闭了服务端 LLM，则直接返回
        if not self.config.get("server", {}).get("enable_server_llm", True):
            self.logger.bind(tag=TAG).debug("服务端 LLM 已禁用，跳过 LLM 调用")
            return None
